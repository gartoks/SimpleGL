...
public class SimplexNoise : INoiseGenerator { // TODO
    private static int[][] GRAD_3 { get; } = {
        new[]{1, 1, 0},
        new[]{-1, 1, 0},
        new[]{1, -1, 0},
        new[]{-1, -1, 0},
    ...
    private static int[][] SIMPLEX { get; } = {
    ...
    public SimplexNoise(IRandomGenerator rng) {
        Seed = rng.Seed;
    ...
    public double Generate(double xin) {
        return Generate(xin, 0);
    }
    ...
    public double Generate(double xin, double yin) {
        ...
        double F2 = 0.5f * (System.Math.Sqrt(3.0f) - 1.0f);
        double s = (xin + yin) * F2; // Hairy factor for 2D
        int i = Fastfloor(xin + s);
        int j = Fastfloor(yin + s);
        double G2 = (3.0f - System.Math.Sqrt(3.0f)) / 6.0f;
        double t = (i + j) * G2;
    ...
    // 3D simplex Noise
    public double Generate(double xin, double yin, double zin) {
    ...
    // 4D simplex Noise
    public double Generate(double xin, double yin, double zin, double win) {
        // The skewing and unskewing factors are hairy again for the 4D case
        double F4 = (System.Math.Sqrt(5.0f) - 1.0f) / 4.0f;
        double G4 = (5.0f - System.Math.Sqrt(5.0f)) / 20.0f;
    ...
    //
    //Blurred noise
    public double BlurredNoise(double stepSize, double x, double y) {
        int totalIterations = 0;
        double noiseSum = 0.0f;

        for (double xx = x - stepSize; xx <= x + stepSize; xx += stepSize) {
            for (double yy = y - stepSize; yy <= y + stepSize; yy += stepSize) {
                noiseSum += Generate(xx, yy);
                totalIterations++;
            }
        }

        return noiseSum / totalIterations;
    }

    public double BlurredNoise(double stepSize, double x, double y, double z, double w) {
        int totalIterations = 0;
        double noiseSum = 0.0f;

        for (double xx = x - stepSize; xx <= x + stepSize; xx += stepSize) {
            for (double yy = y - stepSize; yy <= y + stepSize; yy += stepSize) {
                for (double zz = z - stepSize; zz <= z + stepSize; zz += stepSize) {
                    for (double ww = w - stepSize; ww <= w + stepSize; ww += stepSize) {
                        noiseSum += Generate(xx, yy, zz, ww);
    ...
    //X, Y is [0..1]
    //public double SeamlessNoise(double x, double y, double dx, double dy, double xyOffset) {
    //    double s = x;
    //    double t = y;

    //    double nx = xyOffset + SinCosLUT.Cos(s * 2.0f * Mathf.PI) * dx / (2.0f * Mathf.PI);
    //    double ny = xyOffset + SinCosLUT.Cos(t * 2.0f * Mathf.PI) * dy / (2.0f * Mathf.PI);
    //    double nz = xyOffset + SinCosLUT.Sin(s * 2.0f * Mathf.PI) * dx / (2.0f * Mathf.PI);
    //    double nw = xyOffset + SinCosLUT.Sin(t * 2.0f * Mathf.PI) * dy / (2.0f * Mathf.PI);
    ...
    private static double Dot(int[] g, double x, double y, double z, double w) => g[0] * x + g[1] * y + g[2] * z + g[3] * w;
}